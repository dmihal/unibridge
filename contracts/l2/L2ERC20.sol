...
/* Contract Imports */
import { UniswapV2ERC20 } from "../libraries/UniswapV2ERC20.sol";
 ...
 * Runtime target: OVM
 */
contract L2ERC20 is UniswapV2ERC20, IL2Token {
    /********************************
     ...
     */
    function initialize(address _l1Address, uint8 _decimals) external override onlyBridge {
     ...
     */
    function withdraw(address _destination, uint256 _amount) external override {
        _burn(msg.sender, _amount);
        IL2TokenBridge(bridge).withdraw(l1Address, _destination, _amount);
    }
     ...
     */
    function migrate(uint256 _amount, address _target) external override {
        _burn(msg.sender, _amount);
        IL2TokenBridge(bridge).migrate(l1Address, _target, msg.sender, _amount);
    }
     ...
     */
    function updateInfo(string memory _newName, string memory _newSymbol) external override onlyBridge {
        name = _newName;
        symbol = _newSymbol;
    }
     ...
     */
    function mint(address _recipient, uint256 _amount) external override onlyBridge {
